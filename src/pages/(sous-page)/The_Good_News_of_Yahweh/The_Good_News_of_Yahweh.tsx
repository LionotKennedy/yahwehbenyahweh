import "./style/the_Good_News_of_Yahweh.css";
import "./style/reponsive.css";
import { Link } from "react-router-dom";

// Importation de toutes les images
import Volume3Img from "./image/Volume_3.png";
import Volume2Img from "./image/Volume_2.png";
import Volume1Img from "./image/Volume_1.png";
import JunJulAug2017Img from "./image/Jun_Jul_Aug_2017_Issue.png";
import MarAprMay2017Img from "./image/Mar_Apr_May_2017_Issue.png";
import OctNovDec2016Img from "./image/Oct_Nov_Dec_2016_Issue.png";
import JulAugSep2016Img from "./image/Jul_Aug_Sep_2016_Issue.png";
import AprMayJun2016Img from "./image/Apr_May_Jun_2016_Issue.png";
import JanFebMar2016Img from "./image/Jan_Feb_Mar_2016_Issue.png";
import NovDec2015Img from "./image/Nov_Dec_2015_Issue.png";
import SepOct2015Img from "./image/Sep_Oct_2015_Issue.png";
import NovDec2014Img from "./image/Nov_Dec_2014_Issue.png";

export function The_Good_News_of_Yahweh() {
  return (
    <div id="backdrop-news" className="good-news-page">
      <div id="pg-banner-news" className=""></div>
      <div id="top-bar-gold-news"></div>
      <div className="pg-title-text-news">
        <p className="p-title-text-news">GAZETY</p>
      </div>
      <div className="book-line-news">
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img className="img-size-news" src={Volume3Img} alt="Volume 3" />
            </div>
            <p className="lnk-info-text-news">Volume 3</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/Volume_3/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/Volume_3/Volume_3.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img className="img-size-news" src={Volume2Img} alt="Volume 2" />
            </div>
            <p className="lnk-info-text-news">Volume 2</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/Volume_2/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/Volume_2/Volume_2.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img className="img-size-news" src={Volume1Img} alt="Volume 1" />
            </div>
            <p className="lnk-info-text-news">Volume 1</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/Volume_1/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/Volume_1/Volume_1.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
      </div>
      <div className="sec-bar-news"></div>
      <div className="book-line-news">
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={JunJulAug2017Img}
                alt="Gazety Jona/Jolay/Aogositra 2017"
              />
            </div>
            <p className="lnk-info-text-news">
              Gazety Jona/Jolay/Aogositra 2017
            </p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2017_Jun-Aug/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2017_Jun-Aug/Good_News_Jun-Jul-Aug.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={MarAprMay2017Img}
                alt="Gazety Marsa/Avrily/Mey 2017"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Marsa/Avrily/Mey 2017</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2017_Mar-May/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2017_Mar-May/Good_News_Mar-Apr-May.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={OctNovDec2016Img}
                alt="Gazety Janoary/Febroary/Marsa 2016"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Oktob/Novam/Desa 2016</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2016_Oct-Dec/index.html#p=1"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2016_Oct-Dec/Good_News_Oct-Nov-Dec.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
      </div>
      <div className="sec-bar-news"></div>
      <div className="book-line-news">
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={JulAugSep2016Img}
                alt="Gazety Jolay/Aogo/Septa 2016"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Jolay/Aogo/Septa 2016</p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2016_Oct-Dec/index.html"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2016_Oct-Dec/Good_News_Oct-Nov-Dec.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={AprMayJun2016Img}
                alt="Gazety Avrily/Mey/Jona 2016"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Avrily/Mey/Jona 2016</p>
            <p className="lnk-info-text-news">Gazety Flipbook</p>
            <p className="lnk-info-text-news">Endrika PDF Adobe</p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={JanFebMar2016Img}
                alt="Gazety Janoary/Febroary/Marsa 2016"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Janoa/Febro/Marsa 2016</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2016_Jan-Mar/index.html#p=1"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2016_Jan-Mar/Good_News_Jan-Feb-Mar.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
      </div>
      <div className="sec-bar-news"></div>
      <div className="book-line-news">
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={NovDec2015Img}
                alt="Gazety Novambra/Desambra 2015"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Novambra/Desambra 2015</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2015_Nov-Dec/index.html#p=1"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2015_Nov-Dec/Good_News_Magazine_Nov-Dec.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={SepOct2015Img}
                alt="Gazety Septambra/Oktobra 2015"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Septambra/Oktobra 2015</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2015_Sept-Oct/index.html#p=1"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2015_Sept-Oct/Good_News_Master_Sep_Oct_2015.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
        <div className="book-cnt-news">
          <div className="book-guide-news">
            <div>
              <img
                className="img-size-news"
                src={NovDec2014Img}
                alt="Gazety Novambra/Desambra 2014"
              />
            </div>
            <p className="lnk-info-text-news">Gazety Novambra/Desambra 2014</p>
            <p className="lnk-info-text-news">
              <a
                href="https://www.yahwehbenyahweh.com/The_Good_News_of_Yahweh/goodnews/2014_Nov-Dec/index.html#p=1"
                target="_blank"
                rel="noopener noreferrer"
              >
                Gazety Flipbook
              </a>
            </p>
            <p className="lnk-info-text-news">
              <a
                href="/The_Good_News_of_Yahweh/goodnews/2014_Nov-Dec/Good_News_Nov-Dec.pdf"
                target="_blank"
                rel="noopener noreferrer"
              >
                Endrika PDF Adobe
              </a>
            </p>
          </div>
        </div>
      </div>
      <div className="sec-bar-news"></div>
      <div className="pg-info-text-news">
        <p className="p-info-text-news">
          HO AN'NY GAZETY TALOHA, TSINDRIO ETO AMBANY
          <br />
          <Link to="/The_Good_News_of_Yahweh/back_issues">
            <span className="lnk-info-text-news">
              tahirin'ny gazety taloha (Mey 2002 - Desambra 2008)
            </span>
          </Link>
        </p>
      </div>
      <div id="top-bar-gold-news"></div>
    </div>
  );
}

export default The_Good_News_of_Yahweh;
