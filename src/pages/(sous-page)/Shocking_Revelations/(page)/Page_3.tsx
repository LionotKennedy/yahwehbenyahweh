import "../style/shocking_Revelations.css";
import "../style/responsive.css";
import { Link } from "react-router-dom";

// Import de toutes les images
import ChroniclesImg from "../image/The_Chronicles_of_The_Two_Witnesses.jpg";
import MissionarySpiritImg from "../image/The_Missionary_Spirit.jpg";
import OvercomeBloodImg from "../image/Overcome_Blood_of_the_Lamb_and_Word_of_Testimony.jpg";
import FirstResurrectionImg from "../image/First_Resurrection_First_Come_First_Served.jpg";
import RedeemedIsraelImg from "../image/The_Redeemed_of_Israel.jpg";
import SubmittingWillImg from "../image/Submitting_To_The_Will_of_Yahweh_Ben_Yahweh.jpg";
import HealingLightImg from "../image/The_Healing_Light.jpg";
import SignOfTimesImg from "../image/The_Sign_Of_The_Times.jpg";
import WhoIsWorthyImg from "../image/Who_Is_Worthy_To_Open_The_Book_And_Loose.jpg";
import YBYComingImg from "../image/YBY_Coming_In_The_Clouds_of_Heaven.jpg";
import BeholdThroneImg from "../image/Behold_A_Throne_Was_Set_In_Heaven.jpg";
import MessageLawKeepersImg from "../image/A_Message_For_Law_Keepers.jpg";
import GoToAntImg from "../image/Go_To_The_Ant.jpg";
import YBYOnlyDoorImg from "../image/YBY_The_Only_Door_To_The_Sheepfold.jpg";
import WhenSunStoodImg from "../image/When_The_Sun_Stood_Still.jpg";
import AppointedWatchmanImg from "../image/YBY_The_Appointed_Watchman.jpg";

export function Page_3() {
  return (
    <>
      <div id="pg-banner-shock"></div>
      <div id="top-bar-gold-shock"></div>
      <div className="pg-title-text-shock">
        <p className="p-title-text-shock">TATITRA SEMINARA</p>
      </div>
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/The_Chronicles">
                <img
                  className="img-size-shock"
                  src={ChroniclesImg}
                  alt="The Chronicles of The Two Witnesses"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/The_Missionary_Spirit">
                <img
                  className="img-size-shock"
                  src={MissionarySpiritImg}
                  alt="The Missionary Spirit"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/Overcome_Blood">
                <img
                  className="img-size-shock"
                  src={OvercomeBloodImg}
                  alt="Overcome: Blood of the Lamb and Word of Testimony"
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-line-break-shock"></div>
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/First_Resurrection">
                <img
                  className="img-size-shock"
                  src={FirstResurrectionImg}
                  alt="First Resurrection: First Come, First Served"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/The_Redeemed">
                <img
                  className="img-size-shock"
                  src={RedeemedIsraelImg}
                  alt="The Redeemed of Israel"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/Submitting_To_The_Will">
                <img
                  className="img-size-shock"
                  src={SubmittingWillImg}
                  alt="Submitting To The Will of Yahweh Ben Yahweh"
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-line-break-shock"></div>
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/The_Healing_Light">
                <img
                  className="img-size-shock"
                  src={HealingLightImg}
                  alt="The Healing Light"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/The_Sign_Of_The_Times">
                <img
                  className="img-size-shock"
                  src={SignOfTimesImg}
                  alt="The Sign Of The Times"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/Who_Is_Worthy">
                <img
                  className="img-size-shock"
                  src={WhoIsWorthyImg}
                  alt="Who Is Worthy To Open The Book And Loose..."
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-line-break-shock"></div>
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/YBY_Coming_In">
                <img
                  className="img-size-shock"
                  src={YBYComingImg}
                  alt="Yahweh Ben Yahweh Coming In The Clouds of Heaven..."
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/Behold_A_Throne">
                <img
                  className="img-size-shock"
                  src={BeholdThroneImg}
                  alt="Behold, A Throne Was Set In Heaven"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/A_Message_For">
                <img
                  className="img-size-shock"
                  src={MessageLawKeepersImg}
                  alt="A Message For Law Keepers... The Blood of The Lamb..."
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-line-break-shock"></div>
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/Go_To_The_Ant">
                <img
                  className="img-size-shock"
                  src={GoToAntImg}
                  alt="Go To The Ant"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/YBY_The_Only_Door">
                <img
                  className="img-size-shock"
                  src={YBYOnlyDoorImg}
                  alt="Yahweh Ben Yahweh The Only Door To The Sheepfold"
                />
              </a>
            </div>
          </div>
        </div>
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/When_The_Sun">
                <img
                  className="img-size-shock"
                  src={WhenSunStoodImg}
                  alt="When The Sun Stood Still"
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-line-break-shock"></div>
      {/* Nouvelle ligne pour la nouvelle image */}
      <div className="book-line-shock">
        <div className="book-cnt-shock">
          <div className="book-guide-shock">
            <div>
              <a href="/Shocking_Revelations/3/YBY_The_Appointed_Watchman">
                <img
                  className="img-size-shock"
                  src={AppointedWatchmanImg}
                  alt="Yahweh Ben Yahweh The Appointed Watchman"
                />
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="pg-info-text-shock">
        <p className="p-info-text-shock">
          <br />
          <Link to="/Shocking_Revelations">
            <span className="lnk-info-text-shock"> 1 </span>
          </Link>
          <Link to="/Shocking_Revelations/2">
            <span className="lnk-info-text-shock"> 2 </span>
          </Link>
          <span className="lnk-info-text-no-ln-shock"> 3 </span>
        </p>
      </div>
      <div id="top-bar-gold-shock"></div>
    </>
  );
}
export default Page_3;
