  ...
  const fetchSectionBackground = async () => {
    try {
      ...
      if (success && data) {
        ...
        const about_yahweh_ben = sortedSectionbg.find(
          (banner) => banner.id === 2 || banner.section_name === "about_yahweh_ben"
        );
  ...
  const handleSaveImage = async (
    ...
    try {
      if (galleryIndex !== undefined && typeof imageData === "string") {
        ...
        toast.warning(`Sauvegarde locale pour ${section} (pas d'API configurée)`);
      }
    } catch (error) {
      console.error("Erreur lors de la sauvegarde:", error);
    }
  ...
  const handleSaveImageSecionBG = async (
    ...
    try {
      if (bannerId && typeof imageDataBG === "string") {
        ...
        toast.warning(`Sauvegarde locale pour ${section_name} (pas d'API configurée)`);
      }
    } catch (error) {
      console.error("Erreur lors de la sauvegarde:", error);
    }
